...
void rank()
{
	...
	RECT r21 = {0,0,350,450};
	RECT r22 = {0,0,1000,450};
	RECT r23 = {0,0,1650,450}; 
	settextcolor(WHITE);
	settextstyle(20,0,"Times New Roman");
	drawtext(_T("项目介绍"), &r21, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    drawtext(_T("队员信息"), &r22, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    drawtext(_T("排行榜"), &r23, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	int j,k,flag = 3;
    ...
    RECT r1 = {0, 850, 1100, 1100};
    	for(i=0;i<10;i++) //将奖牌榜的国家和奖牌数量读入到字符串中，再输出 
	{
		...
		//outtextxy(100, j,str);
		drawtext(_T(str), &r, DT_VCENTER | DT_CENTER );  
		r.top += 48; //向下平移 
		r.bottom += 48;
...
while (true)
	{
		ExMessage msg,msg3;    //消息结构体 
		flushmessage();   //清空当前消息队列所有消息 
		msg = getmessage(EM_MOUSE|EM_KEY); 
		msg3 = getmessage(EM_MOUSE|EM_KEY);   //获取消息队列中的 鼠标消息和按键消息 ，具体可查看Easy_X操作文档 
    ...
    flushmessage();
    if(msg3.vkcode==VK_RIGHT)
    {
    	
    	rank1();
	}	
	if(msg3.vkcode==VK_LEFT)
    {
...
while (true)
	{
		ExMessage msg,msg2;    //消息结构体 
		flushmessage();   //清空当前消息队列所有消息 
		msg = getmessage(EM_MOUSE|EM_KEY);  
		msg2 = getmessage(EM_MOUSE|EM_KEY);  //获取消息队列中的 鼠标消息和按键消息 ，具体可查看Easy_X操作文档 
		if(msg.message==WM_LBUTTONDOWN)      //点击鼠标左键 ，WM_LBUTTONDOWN是Windows消息，具体可百度了解 
		{
	...
	if(msg2.vkcode==VK_LEFT)
	{
		rank1();
	}
	if(msg2.vkcode==VK_RIGHT)
	{
		rank();
	}
